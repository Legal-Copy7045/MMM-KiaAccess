...
# the HA integration must carry copies of these kept in sync by
# scripts/sync-core.js -- byte-identical for JSON, and for .py files the
# synced copy also carries a "GENERATED, edit the root copy" banner
# (scripts/sync-core.js's PY_BANNER) prepended ahead of the otherwise
# byte-identical content, so a stray edit made directly in
# custom_components/kia_access/*.py doesn't look like an ordinary,
# hand-editable file.
PY_BANNER = (
...
for name in ("entities.json", "commands.json", "kia_client.py",
    ...
    expected = src if name.endswith("json") else PY_BANNER + src
    assert dst == expected, f"{name} out of sync — run node scripts/sync-core.js"
